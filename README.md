...
By default, Copilot might use `dotnet new` directly instead of the MCP tools. To make it automatically use `template_from_intent`, `template_instantiate`, etc., add a `.github/copilot-instructions.md` to your workspace:
...
This works per-workspace. To apply it globally, add the same text to your VS Code user settings under `github.copilot.chat.codeGeneration.instructions`.
