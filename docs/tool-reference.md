...
- **Post-actions** — runs the template's built-in restore (`dotnet restore`) and add-to-solution (`dotnet sln add`) post-actions, so you get the same finished state as `dotnet new`. Template-supplied *scripts* and process-start actions are **never** executed automatically; they are reported with their manual instructions instead. Note that this is not the same as "no template-authored logic runs": `dotnet restore` performs a full MSBuild evaluation of the generated project, so a template that emits a custom target or `Directory.Build.props` can still cause code to execute — the same exposure you accept when running `dotnet new` followed by a restore. Disable with `runPostActions=false` or `MCP_TEMPLATE_POST_ACTIONS=false`.
- **NuGet version reporting** — queries the feeds configured in your `NuGet.config` (private feeds, credentials, `packageSourceMapping` and proxies all honored) and **reports** newer stable versions without modifying the project. Pass `resolveLatestVersions=true` to apply them, or `resolveLatestVersions=false` to skip feed lookups entirely (no network access). `MCP_TEMPLATE_OFFLINE=true` makes skipping the default.
